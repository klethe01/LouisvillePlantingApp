...
# single lowest and highest hourly readings (raw float) across
# the full 14-day window for air temp and soil temp at 6cm.
#
# Risk logic:
#   HIGH:   min_air < 40°F (air too cold at any point)
#           OR min_soil < plant.min_soil_temp_6cm (soil too cold at any point)
#           OR max_soil > plant.max_soil_temp_6cm (soil too hot at any point)
#
#   LOW:    soil >= plant.opt_soil_temp_6cm for the whole window (if opt exists)
#           OR soil >= plant.min_soil_temp_6cm for the whole window (if no opt)
#           AND soil <= plant.max_soil_temp_6cm AND air >= 40°F
#
#   MEDIUM: soil >= plant.min_soil_temp_6cm but < plant.opt_soil_temp_6cm
#           AND soil <= plant.max_soil_temp_6cm AND air >= 40°F
#           (only possible for plants that have an opt_soil_temp_6cm value)
# ============================================================
...
def _score_risk(
    ...
    Parameters:
        min_soil      — lowest soil temp recorded across the 14-day window (°F)
        max_soil      — highest soil temp recorded across the 14-day window (°F)
        min_air       — lowest air temp recorded across the 14-day window (°F)
        min_threshold — plant's minimum soil temp for germination (°F)
        opt_threshold — plant's optimal soil temp (OFA source); None if unavailable
        max_threshold — plant's maximum soil temp for germination (°F)

    Tiers:
        HIGH   — air dropped below 40°F at any point, OR soil dropped below
                 minimum at any point, OR soil exceeded maximum at any point.
        LOW    — soil was at or above optimum for the whole window (if opt exists),
                 OR at or above minimum for the whole window (if no opt listed),
                 AND soil stayed below maximum, AND air stayed at or above 40°F.
        MEDIUM — soil was at or above minimum and below maximum for the whole
                 window, AND air was at or above 40°F, BUT soil did not yet reach
                 the optimum level. Only possible for plants with an opt value.
    """
    # HIGH RISK — air too cold at any point in the 14-day window
    if min_air < UNIVERSAL_MIN_AIR_TEMP_F:
        return (
            "high",
            (
                f"Air temperature ({min_air:.1f}°F) dropped below the minimum "
                f"safe threshold ({UNIVERSAL_MIN_AIR_TEMP_F}°F) at some point "
                f"in the 14-day window. Not recommended."
            ),
        )

    # HIGH RISK — soil too cold at any point (below plant minimum)
    if min_soil < min_threshold:
        return (
            "high",
            (
                f"Soil temperature ({min_soil:.1f}°F) dropped below the minimum "
                f"required for germination ({min_threshold:.1f}°F) at some point "
                f"in the 14-day window. Not recommended."
            ),
        )

    # HIGH RISK — soil too hot at any point (above plant maximum)
    if max_soil > max_threshold:
        return (
            "high",
            (
                f"Soil temperature ({max_soil:.1f}°F) exceeded the maximum "
                f"safe temperature for germination ({max_threshold:.1f}°F) at "
                f"some point in the 14-day window. Not recommended."
            ),
        )

    # From here: air >= 40°F, soil >= minimum, soil <= maximum for full window.

    # Plants with no OFA optimum listed go straight to Low Risk
    # once soil is above their minimum (no Medium zone for these plants).
    if opt_threshold is None:
        return (
            "low",
            (
                f"Soil temperature ({min_soil:.1f}°F) is within the safe "
                f"germination range ({min_threshold:.1f}–{max_threshold:.1f}°F) "
                f"and air temperature ({min_air:.1f}°F) is at or above "
                f"{UNIVERSAL_MIN_AIR_TEMP_F}°F throughout the 14-day window. "
                f"Recommended."
            ),
        )

    # LOW RISK — soil at or above optimum for the whole window
    if min_soil >= opt_threshold:
        return (
            "low",
            (
                f"Soil temperature ({min_soil:.1f}°F) is at or above the optimal "
                f"level ({opt_threshold:.1f}°F) and within the safe maximum "
                f"({max_threshold:.1f}°F), with air temperature ({min_air:.1f}°F) "
                f"at or above {UNIVERSAL_MIN_AIR_TEMP_F}°F. Recommended."
            ),
        )

    # MEDIUM RISK — soil above minimum but not yet at optimum
    return (
        "medium",
        (
            f"Soil temperature ({min_soil:.1f}°F) is above the minimum "
            f"({min_threshold:.1f}°F) but has not yet reached the optimal level "
            f"({opt_threshold:.1f}°F). Air temperature ({min_air:.1f}°F) is safe. "
            f"May advise waiting for further soil warming."
        ),
    )
...
def compute_risk(cur, temps_clean: pd.DataFrame) -> list[RiskRow]:
    ...
    def _max_or_fail(series: pd.Series, label: str) -> float:
    ...
    max_soil_6cm = _max_or_fail(temps_clean["soil_6cm_raw"],  "6cm soil temperature")
    ...
    log.debug(
        "  14-day values — air min: %.1f°F | soil min: %.1f°F | soil max: %.1f°F",
        min_air, min_soil_6cm, max_soil_6cm,
    )

    # Fetch all plants and their soil temperature thresholds from the DB.
    # opt_soil_temp_6cm may be NULL for plants without OFA optimum data.
    cur.execute("""
        SELECT
            plant_id, common_name,
            min_soil_temp_6cm, opt_soil_temp_6cm, max_soil_temp_6cm
        FROM plants
        ORDER BY plant_id
    """)
    plants = cur.fetchall()
    ...
    for (
        plant_id, common_name,
        min_6cm, opt_6cm, max_6cm,
    ) in plants:

        risk_level, risk_desc = _score_risk(
            min_soil      = min_soil_6cm,
            max_soil      = max_soil_6cm,
            min_air       = min_air,
            min_threshold = min_6cm,
            opt_threshold = opt_6cm,    # None for plants without OFA optimum
            max_threshold = max_6cm,
        )
        risk_rows.append({
            "plant_id":          plant_id,
