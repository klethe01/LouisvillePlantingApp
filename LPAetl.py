...
Usage:
    Standalone:  python lpa_etl.py
    From Dash:   from lpa_etl import run_etl
...
Environment (.env file in same directory):
    ...
    DB_SSL=require
    LOG_FILE=lpa_etl.log   (optional, defaults to lpa_etl.log)
...
LOG_FILE  = os.getenv("LOG_FILE",  "lpa_etl.log")
...
def extract_temperatures() -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            f"Open Meteo API timed out after {API_TIMEOUT_SECONDS}s."
        )
    ...
    # Confirm the expected 'hourly' structure is present before returning
    if "hourly" not in raw:
        raise RuntimeError(
            f"Unexpected API response — 'hourly' key missing. "
            f"Keys found: {list(raw.keys())}"
        )

    required_vars = {"time", "temperature_2m", "soil_temperature_6cm", "soil_temperature_18cm"}
    missing_vars = required_vars - set(raw["hourly"].keys())
    if missing_vars:
        raise RuntimeError(
...
def clean_temperatures(raw: dict) -> pd.DataFrame:
    ...
    # Localize to America/New_York, then convert to UTC for consistent storage.
    def parse_to_utc(ts_str: str) -> datetime:
        naive = datetime.fromisoformat(ts_str)
        eastern = naive.replace(tzinfo=TZ_EASTERN)
        return eastern.astimezone(TZ_UTC)

    df["timestamp"] = df["timestamp_str"].apply(parse_to_utc)
...
def _score_risk(
    ...
    Args:
        min_soil:           lowest soil temp recorded in the 14-day window (°F)
        min_air:            lowest air temp recorded in the 14-day window (°F)
        min_soil_threshold: plant's minimum soil temp requirement (°F)
        opt_soil_threshold: plant's optimal soil temp (°F)
        min_air_threshold:  plant's minimum air temp requirement (°F)
        opt_air_threshold:  plant's optimal air temp (°F)
        planting_type:      'seed' or 'transplant' (used in the description)

    Returns:
        (risk_level, risk_desc): a tuple of the risk tier string and
        a human-readable explanation for display in the Dash app.
    """
...
def compute_risk(cur, temps_clean: pd.DataFrame) -> list[dict]:
    """
    ...
    min_air       = float(temps_clean["air_temp_raw"].min())
    min_soil_6cm  = float(temps_clean["soil_6cm_raw"].min())
    min_soil_18cm = float(temps_clean["soil_18cm_raw"].min())
    ...
    for (
        ...
        log.debug("  %-20s | seed      → %s", common_name, seed_level)
        ...
        if min_18cm is None or opt_18cm is None:
            log.debug(
                "  %-20s | transplant → skipped (no 18cm threshold data)",
                common_name,
            )
            skipped_count += 1
            continue
        ...
        log.debug("  %-20s | transplant → %s", common_name, transplant_level)
...
def validate_temps(temps_clean: pd.DataFrame) -> bool:
    ...
    # Critical check: row count
    # 14 days × 24 hours = 336. Allow 10% tolerance for API edge cases
    # (e.g. DST transitions or very recent API calls near midnight).
    expected = 14 * 24
...
def validate_risk(risk_rows: list[dict]) -> bool:
    """
...
#
#   load_risk: deletes all existing risk rows and replaces
#       them with the freshly computed assessments.
#       Delete-then-insert ensures no stale results remain
#       even if the plant list changes between runs.
# ============================================================

def load_temps(cur, temps_clean: pd.DataFrame) -> int:
    """
    ...
    """
    log.info("STAGE 5 — Load: upserting %d rows into temps...", len(temps_clean))

    # Build list of tuples for execute_values.
    # Null-safe conversion: pd.isna() handles both float NaN and pd.NA
    # (from nullable Int64 dtype), converting them to Python None
    # so psycopg2 inserts NULL rather than crashing.
    rows = [
        (
            row["timestamp"],
            bool(row["is_forecast"]),
            None if pd.isna(row["air_temp"])       else int(row["air_temp"]),
            None if pd.isna(row["soil_6cm_temp"])  else int(row["soil_6cm_temp"]),
            None if pd.isna(row["soil_18cm_temp"]) else int(row["soil_18cm_temp"]),
        )
        for _, row in temps_clean.iterrows()
    ]
    ...
    # These are left over from previous pipeline runs and are no longer
    # relevant to the current risk assessment.
    cur.execute("""
        DELETE FROM temps
        WHERE timestamp < NOW() AT TIME ZONE 'UTC' - INTERVAL '7 days'
           OR timestamp > NOW() AT TIME ZONE 'UTC' + INTERVAL '7 days'
    """)
    ...
    log.info("  temps load complete: %d rows upserted.", len(rows))
    return len(rows)
...
def run_etl() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        # Stage 4 (pre-load validation): validate temps before touching the DB.
        # Run here — before opening a DB connection — so a bad API response
        # doesn't start a transaction unnecessarily.
        ...
        with get_db_connection() as conn:
            ...
            # Stage 4 (risk validation): validate risk rows before loading
            if not validate_risk(risk_rows):
                raise RuntimeError(
                    "Risk rows failed validation. Pipeline aborted."
                )

            # Stage 5: Load temps (upsert) and risk (replace) into DB
            load_temps(cur, temps_clean)
