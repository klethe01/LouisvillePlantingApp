...
CREATE TABLE plants (
    ...
    opt_soil_temp_6cm   FLOAT    CHECK (
                                     opt_soil_temp_6cm IS NULL
                                     OR (opt_soil_temp_6cm >= min_soil_temp_6cm
                                         AND opt_soil_temp_6cm <= max_soil_temp_6cm)
                                 ),
    max_soil_temp_6cm   FLOAT    NOT NULL CHECK (max_soil_temp_6cm > min_soil_temp_6cm),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
COMMENT ON COLUMN plants.min_soil_temp_6cm IS 'Minimum soil temp in F at 6cm for seed germination. Below this = High Risk.';
COMMENT ON COLUMN plants.opt_soil_temp_6cm IS 'Optimal soil temp in F at 6cm (OFA source). NULL if not available. At or above this = Low Risk.';
COMMENT ON COLUMN plants.max_soil_temp_6cm IS 'Maximum soil temp in F at 6cm for germination. Above this = High Risk.';
...
#    min_soil_6cm, opt_soil_6cm, max_soil_6cm)
#
# opt_soil_6cm = OFA Optimum lower bound (None if not in OFA data).
#   Plants WITH opt: Low Risk when soil >= opt, Medium Risk when >= min but < opt.
#   Plants WITHOUT opt: Low Risk once soil >= min (no Medium zone).
# max_soil_6cm = Chart Max from backwoodsenergy.org (or OFA Max where unavailable).
#   Soil above max at any point = High Risk (too hot for germination).
#
...
PLANTS = [
    # (common_name, category, min_soil_6cm, opt_soil_6cm, max_soil_6cm)
    # opt_soil_6cm = OFA Optimum lower bound; None if no OFA optimum data available
    # max_soil_6cm = Chart Max (backwoodsenergy.org) or OFA Max where chart unavailable
    #
    # Vegetables / fruit
    ("asparagus",       "vegetable/fruit",  50,   None, 84),
    ("bean",            "vegetable/fruit",  60,   60,   95),
    ("beet",            "vegetable/fruit",  40,   50,   85),
    ("blackberry",      "vegetable/fruit",  45,   None, 74),
    ("cabbage",         "vegetable/fruit",  45,   45,   85),
    ("carrot",          "vegetable/fruit",  45,   45,   95),
    ("celery",          "vegetable/fruit",  60,   60,   85),
    ("chard",           "vegetable/fruit",  50,   50,   95),
    ("collard",         "vegetable/fruit",  45,   None, 76),
    ("cucumber",        "vegetable/fruit",  60,   60,   105),
    ("eggplant",        "vegetable/fruit",  60,   75,   95),
    ("gourds",          "vegetable/fruit",  70,   None, 90),
    ("ground cherry",   "vegetable/fruit",  65,   None, 84),
    ("leek",            "vegetable/fruit",  50,   None, 76),
    ("lettuce",         "vegetable/fruit",  35,   40,   80),
    ("melon",           "vegetable/fruit",  70,   70,   86),
    ("okra",            "vegetable/fruit",  65,   None, 90),
    ("onion",           "vegetable/fruit",  35,   50,   82),
    ("parsnip",         "vegetable/fruit",  35,   50,   74),
    ("sweet pea",       "vegetable/fruit",  40,   40,   78),
    ("southern pea",    "vegetable/fruit",  60,   None, 95),
    ("pepper",          "vegetable/fruit",  65,   65,   95),
    ("pumpkin",         "vegetable/fruit",  60,   70,   90),
    ("radish",          "vegetable/fruit",  40,   None, 88),
    ("sorghum",         "vegetable/fruit",  60,   None, 90),
    ("spinach",         "vegetable/fruit",  35,   45,   85),
    ("squash",          "vegetable/fruit",  60,   70,   90),
    ("strawberry",      "vegetable/fruit",  50,   None, 76),
    ("sweet corn",      "vegetable/fruit",  50,   60,   105),
    ("tomatillo",       "vegetable/fruit",  65,   None, 95),
    ("tomato",          "vegetable/fruit",  60,   70,   88),
    ("turnip",          "vegetable/fruit",  40,   60,   95),
    # Flowers — no OFA optimum data available for any flower
    ("cosmos",          "flower",           70,   None, 74),
    ("marigold",        "flower",           65,   None, 74),
    ("senna",           "flower",           65,   None, 80),
    ("sunflower",       "flower",           55,   None, 84),
    ("zinnia",          "flower",           70,   None, 84),
    # Herbs — no OFA optimum data available for any herb
    ("basil",           "herb",             65,   None, 84),
    ("chives",          "herb",             50,   None, 70),
    ("cilantro",        "herb",             55,   None, 70),
    ("dill",            "herb",             60,   None, 70),
    ("mint",            "herb",             55,   None, 74),
    ("mustard",         "herb",             40,   None, 66),
    ("oregano",         "herb",             65,   None, 76),
    ("parsley",         "herb",             50,   None, 84),
    ("sage",            "herb",             60,   None, 70),
    ("thyme",           "herb",             60,   None, 74),
]
...
def verify(cur) -> None:
    ...
    cur.execute("""
        ...
        WHERE max_soil_temp_6cm <= min_soil_temp_6cm
           OR (opt_soil_temp_6cm IS NOT NULL
               AND (opt_soil_temp_6cm < min_soil_temp_6cm
                    OR opt_soil_temp_6cm > max_soil_temp_6cm))
    """)
    bad = cur.fetchall()
    if bad:
...
def main() -> None:
    ...
    try:
        ...
        plant_rows = [
            (name, cat_map[cat], min6, opt6, max6)
            for name, cat, min6, opt6, max6 in PLANTS
        ]
